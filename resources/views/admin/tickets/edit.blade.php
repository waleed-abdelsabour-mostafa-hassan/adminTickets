    ...
    <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
                ...
                <!-- end breadcrumbs -->
                <h1 class="page-header"> {{ __('pages.main.Edit_ticket') }} : {{$ticket->name}}</h1>
            </div>
            <!-- /.col-lg-12 -->
            <div class="col-xs-8">
                <form action="{{route('tickets.update',$ticket->id)}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control col-xs-8 input-lg createpro"
                            name="title" placeholder=" العنوان ..." value="{{$ticket->title}}">
                        @error('title')
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control col-xs-8 input-lg createpro" rows="5" name="description" placeholder=" التفاصيل ...">{{$ticket->description}}</textarea>
                        @error('description')
                            <span class="invalid-feedback" role="alert">
                                <h4 class="text-danger text-center col-xs-8 col-xs-offset-4">{{ $message }}</h4>
                            </span>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control col-xs-8 input-lg createpro" type="date"
                            name="from_date" value="{{$ticket->from_date}}">
                        @error('from_date')
                            <span class="invalid-feedback" role="alert">
                                <h4 class="text-danger text-center col-xs-8 col-xs-offset-4">{{ $message }}</h4>
                            </span>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control col-xs-8 input-lg createpro" type="date"
                            name="to_date" value="{{$ticket->to_date}}">
                        @error('to_date')
                            <span class="invalid-feedback" role="alert">
                                <h4 class="text-danger text-center col-xs-8 col-xs-offset-4">{{ $message }}</h4>
                            </span>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control col-xs-8 input-lg createpro selecton"
                            ...
                            @foreach(App\User::where([['type','admin'],['id','!=',Auth::user()->id]])->get() as $user)
                                <option value="{{ $user->id }}" {{($ticket->assigned_id== $user->id)? 'selected':''}}>
                                    {{ $user->name }}</option>
                            @endforeach
                        </select>
                    </div>
